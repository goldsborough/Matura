...
Liegt der Mittelpunkt eines Kreises mit Radius $r = 1$ im Koordinatenursprung $(0, 0)$, erh\"{a}lt man die sehr kompakte und einpr\"{a}gsame Kreisgleichung des \emph{Einheitskreises}: $$k_{E}:\,x^2 + y^2 = 1$$ Multipliziert man die Kreisgleichung aus, erh\"{a}lt man die \textbf{allgemeine Kreisgleichung}:
...
Eine Ellipse entsteht wenn ein Kegel von einer Ebene geschnitten wird, die schr\"{a}g zur Basis steht. Sie ist definiert durch zwei Brennpunkte $F_{1}$ und $F_{2}$ sowie einer Zahl $a > 0$ oder einem beliebigen Punkt $P$ auf der Ellipse. Eine vollst\"{a}ndig beschriftete Ellipse $ell$ mit einem Punkt $X$ sieht so aus:
...
Eine Parabel ist eine Hyperbel mit nur einem Bogen bzw. mit nur einem Ast. Die Kurve einer Parabel ist exakt definiert durch einen Brennpunkt $F$ sowie eine Leitlinie $l$. Liegt der Brennpunkt $F$ auf der positiven $x$-Achse, der Scheitel der Parabel im Koordinatenursprung und ist die Leitlinie $l$ parallel zur $y$-Achse, handelt es sich um eine Parabel in \textbf{1. Hauptlage}. In diesem Fall ist die Parabel schon durch den Parameter $p$ exakt definiert. Dieser Parameter $p$ bestimmt den Abstand zwischen dem Brennpunkt $F$ und der Leitlinie $l$ und ist daher doppelt so gro\ss{} wie die lineare Exzentrizit\"{a}t $e$.
