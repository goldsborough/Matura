...
\begin{itemize}
	\bolditem{gerade bzw. achsensymmetrisch}, wenn die Funktion and der $y$-Achse gespiegelt ist sodass gilt $f(-x) = f(x)$
...
\textbf{Hauptform} \defas $x^z + px + q = 0$

\textbf{Allgemeine Form} \defas $ax^z + bx + c = 0$
...
\textbf{Allgemeine Form} \defas $\sum_{i=0}^{n} a_{i}x^i$
...
\textbf{Allgemeine Form} \defas $ab^x$
...
\textbf{Wachstums- bzw. Zerfallsfunktionsgleichung} \defas $N(t) = N_{0} = e^{\lambda \cdot t}$
