...
Mittels dem durch das Kreuzprodukt gefundenen Normalvektor kann man auch eine Ebene in der Normalvektorform darstellen: $$\epsilon: X * \vec{n_{\epsilon}} = \vec{0A} * \vec{n_{\epsilon}}$$ $$\colvec{x\\y} * \colvec{x_{n_{\epsilon}}\\y_{n_{\epsilon}}} = \colvec{x_{A} \\ y_{A}} * \colvec{x_{n_{\epsilon}}\\y_{n_{\epsilon}}}$$
...
\begin{enumerate}
  ...
  Sind die Gerade $g$ und Ebene $\epsilon$ in Parameterform gegeben, kann man sie gleichsetzen und das daraus entstehende Gleichungssystem l\"{o}sen.
  $$g: \vec{0X} = \colvec{x_A \\ y_A \\ z_A} + u \cdot \colvec{x_u \\ y_u \\ z_u}$$ 

  $$\epsilon: \vec{0X} = \colvec{x_E \\ y_E \\ z_E} + s \cdot \colvec{x_s \\ y_s \\ z_s} + t \cdot \colvec{x_t \\ y_t \\ z_t}$$ 
  ...
  Ist die Gerade $g$ in Parameterform aber die Ebene $\epsilon$ in Normalvektorform, muss man die $x, y und z$ Parameter der Gerade in die Normalvektorform einsetzen. $$g: \vec{0X} = \colvec{x_A \\ y_A \\ z_A} + t \cdot \colvec{x_t \\ y_t \\ z_t}$$

  $$x_g = x_A \cdot t \cdot x_t$$ $$y_g = y_A \cdot t \cdot y_t$$ $$z_g = z_A \cdot t \cdot z_t$$

  $$x_g, y_g, z_g \text{ in } \epsilon: ax + by + cz = d$$ $$ a(x_A \cdot t \cdot x_t) + b(y_A \cdot t \cdot y_t) + c(z_g = z_A \cdot t \cdot z_t) = d$$
...
Die Lagebeziehungen von zwei Ebenen $\epsilon_{1}$ und $\epsilon_{2}$ $$\epsilon_{1}: a_{1}x + b_{1}y + c_{1}z = d_{1}$$ $$\epsilon_{2}: a_{2}x + b_{2}y + c_{2}z = d_{2}$$ im Raum sind gleich wie jene f\"{u}r Geraden in der Ebene (Dimensionunterschied jeweils $= 1$): ident, parallel oder schneidend (kein windschief).

\begin{itemize}

  \item{\textbf{Parallel}}

  Zwei Ebenen $\epsilon_{1}$ und $\epsilon_{2}$ sind dann parallel, wenn ihre Normalvektoren Vielfache von einander sind. Den Normalvektor kann man an den Koeffizienten der Variablen $x, y$ und $z$ ablesen.
  $$\epsilon_{1}: 4x - 3y + 5z = 7 \Rightarrow \vec{n}_{1} = \colvec{4 \\ -3 \\ 5}$$
  $$\epsilon_{2}: 12x -9y + 15z = 6 \Rightarrow \vec{n}_{2} = \colvec{12 \\ -9 \\ 15}$$
  $$n_{1} \cdot 3 = n_{2}$$
  ...
  $$\epsilon_{1}: 4x - 3y + 5z = 2$$
  $$\epsilon_{2}: 12x -9y + 15z = 6$$
  $$\epsilon_{1} \cdot 3 = \epsilon_{2}$$

  \item{\textbf{Schneidend}}

  Letztlich k\"{o}nnen sich zwei Ebenen $\epsilon_{1}$ und $\epsilon_{2}$ noch schneiden und somit eine Schnittgerade bilden. Man erkennt, ob sich zwei Ebenen schneiden, daran, dass sie weder parallel noch ident sind. Da dreidimensionale Ebenen drei Parameter $x, y$ und $z$ besitzen, ist ein Gleichungssystem zwischen zwei Ebenen nicht definitiv l\"{o}sbar. Mann setzt daher den letzten Parameter $z = t$ und berechnet somit die $x, y$ und $z$ Werte in Abh\"{a}ngigkeit des Parameters $t$, wodurch man folglich eine Gerade in Parameterform bilden kann.

  $$\epsilon_{1}: x - 2y + 2z = 3$$ $$\epsilon_{2}: 2x + y - z = 1$$

  \begin{table}[h!]
    \begin{tabular}{l l}
    I. Gleichsetzen von $z$ mit dem Parameter $t$: & $z = t$
    \\
    II. Einsetzen von $t$ in $\epsilon_{1}$: & $\epsilon_{1}: x - 2y + 2t = 3$
    \\
    III. Einsetzen von $t$ in $\epsilon_{2}$: & $\epsilon_{2}: 2x + y - t = 1$
    \\
    IV. L\"{o}sen nach $y$: & $\epsilon_{1} \cdot (-2) \cap \epsilon_{2} \Rightarrow y = -1 + t$
    \\
    V. Finden von $x$: & $y$ in $\epsilon_{2} \Rightarrow x = 1$
    \\
...
\begin{enumerate}
  ...
  F\"{u}r die erste Methode ist es notwendig, den Richtungsvektor $\vec{g}$ der Gerade als Normalvektor einer virtuellen Ebene $\epsilon$ zu sehen, welche durch den Punkt $P$ geht. $$g: \vec{0X} = \vec{0A} + t \cdot \vec{g} \Rightarrow \vec{g} = \vec{n_{\epsilon}}$$ $$\epsilon: \vec{0X} \cdot \vec{n_{\epsilon}} = \vec{n_{\epsilon}} \cdot \vec{0P}$$
...
Ebenso gibt es zur Berechnung des Abstandes eines Punktes $P$ zu einer Ebene $\epsilon$ zwei M\"{o}glichkeiten:

\begin{enumerate}
  \item{\textbf{Gerade mit Normalvektor}}

  In diesem Fall berechnet man zuerst den Normalvektor $\vec{n_{\epsilon}}$ der Ebene, und stellt dann eine Gerade $g$ auf, die durch den Punkt $P$ geht: $$g: \vec{0X} = \vec{0P} + t \cdot \vec{n_{\epsilon}}$$ Diese Gerade schneidet man dann mit der Ebene, um einen Schnittpunkt $S$ zu erhalten. Der gesuchte Abstand $d$ ist dann der Betrag des Vektors $\vec{SP}$

  \item{\textbf{Hesse'sche Abstandsformel}}

  Ebenso ist es m\"{o}glich, mit Hilfe der Hesse'schen Abstandsformel bzw. der Normalprojektion den Abstand $d$ zu berechnen. Hierbei berechnet man wieder den Normalvektor $\vec{n_{\epsilon}}$ der Ebene $\epsilon$ und stellt so eine Gerade $g$ durch den Punkt $P$ auf. Ebenso ben\"{o}tigt man den Einheitsvektor $\vec{n_{0}}$ des Normalvektors der Ebene, sowie einen beliebig gew\"{a}hlten Punkt $A$ auf der Ebene. Die Hesse'sche Abstandsformel lautet dann: $$d = |\vec{PA} \cdot \vec{n_{0}}|$$
